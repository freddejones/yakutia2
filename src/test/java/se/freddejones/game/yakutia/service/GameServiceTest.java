...
//    }
    ...
    private void setupValidNumberOfPlayersInMock() {
        ...
        gamePlayers.add(gamePlayerMock);
        gamePlayers.add(gamePlayerMock);
        when(gamePlayerDaoMock.getGamePlayersByGameId(new GameId(GAME_ID))).thenReturn(gamePlayers);
    }

    private void setupGetGamesForPlayerDefaultMockSettings() {
        when(gamePlayerDaoMock.getGamePlayersByPlayerId(any(PlayerId.class))).thenReturn(
                new TestBoilerplate.GamePlayersListBuilder().addGamePlayer(gamePlayerMock).build());
        when(gamePlayerDaoMock.getGamePlayerByGameIdAndPlayerId(any(PlayerId.class), any(GameId.class))).thenReturn(gamePlayerMock);
        ...
        when(gameDaoMock.getGameByGameId(new GameId(GAME_ID))).thenReturn(gameMock);
